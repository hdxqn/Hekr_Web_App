...
function setPowerState(e) {
    3 == e && (e = 2), $("#funcPower").data("power", e), $("#funcPower")[2 == e ? "removeClass" : "addClass"]("off"), $("#funcMode")[2 == e ? "removeClass" : "addClass"]("off"), $("#funcMode").off("click", onFuncMode), $("#funcMode")[2 == e ? "on" : "off"]("click", onFuncMode)
}
...
SEXP.parse = function(e) {
    ...
    function e(t, n, a) {
        ...
        u.addEventListener("open", function(e) {
            ...
            c.onopen = function(n) {
                clearTimeout(o), (s.debug || e.debugAll) && console.debug("ReconnectingWebSocket", "onopen", s.url), s.protocol = c.protocol, s.readyState = WebSocket.OPEN, s.reconnectAttempts = 0;
                var a = r("open");
...
var tid = getUrlParam("tid"), host = getUrlParam("host") || "device.smartmatrix.mx", token = getUrlParam("access_key"), user = getUrlParam("user") || randomString(10), url = "ws://{host}:8080/websocket/t/{user}/code/{token}/user".format({host: host,user: user,token: token}), ws = new ReconnectingWebSocket(url);
ws.onmessage = function(e) {
    ...
    i18n.init({detectLngQS: "lang",fallbackLng: "en"}, function(e) {
        ...
        $("#back").click(function(e) {
            ...
            var n = '(@devcall "{tid}" (controlModel {args}) (lambda (x) x))'.replace("{tid}", tid), a = $(this).data("power"), r = toggle12(a), o = n.replace("{args}", r);
            console.debug("[CODE] " + o), ws.send(o), t.show()
        }), window.onFuncMode = function() {
            var e = '(@devcall "{tid}" (controlModel {args}) (lambda (x) x))'.replace("{tid}", tid), n = $("#funcMode").data("mode"), a = toggle23(n), r = e.replace("{args}", a);
            console.debug("[CODE] " + r), ws.send(r), t.show()
        }, $("#funcWC").click(function(e) {
            var n = '(@devcall "{tid}" (controlWC {args}) (lambda (x) x))'.replace("{tid}", tid), a = 1 === parseInt($(this).data("wc")) ? 2 : 1, r = n.replace("{args}", a);
            console.debug("[CODE] " + r), ws.send(r), t.show()
        ...
        $("#funcH").change(function(e) {
            var n = '(@devcall "{tid}" (controlH {args}) (lambda (x) x))'.replace("{tid}", tid), a = n.replace("{args}", e.target.value);
            console.debug("[CODE] " + a), ws.send(a), t.show(), 
            $("#toHumidity").text(e.target.value);
            ...
            $("#timers>button").addClass("off"), $(this).removeClass("off");
            var t = $("#timers>button").index(this) + 2, n = $("#funcTime").val();
            $("#timers").data("mode", t), $("#timerLabel").text(getTimerLabel(t, n))
